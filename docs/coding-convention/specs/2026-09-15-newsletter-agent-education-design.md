...
### 3.1 채택 소스 (5개 기관·매체, 6개 RSS 피드, 연결 상태 직접 확인 완료)

| 소스 | URL | 확인 결과 |
|---|---|---|
| 베리타스알파 · 대입 섹션 + 전체기사 (2개 피드) | `https://www.veritas-a.com/rss/S1N2.xml`, `https://www.veritas-a.com/rss/allArticle.xml` | HTTP 200 (둘 다). 전체기사만 쓰다가, RSS가 최신 50건만 유지해 대입과 무관한 기사가 많이 올라오면 관련 기사가 금방 밀려나는 걸 실행 중 발견 — 대입 섹션(안정적으로 남음) + 전체기사(넓은 커버리지)를 함께 수집하도록 변경. URL 기준 중복 제거로 겹치는 기사는 한 번만 집계됨 |
...
→ **결론: 공식 발표(교육부·대교협·KICE) 소스는 이번 제출 범위에서 제외**하고, 전문 매체 3개(베리타스알파 2개 피드 포함) + KEDI 보도자료 1개(정책·통계 콘텐츠로 공식 발표에 가장 근접) + 서울특별시교육청 1개(16개 시도교육청 중 유일한 공식 RSS) 총 5개 기관·매체(6개 피드)로 MVP를 구성한다. MOE/KICE는 목록이 JS로 렌더링되는 구조라 `requests`만으로 수집이 불가능해 브라우저 기반 정찰이 필요하며, 이는 §11 스트레치 목표로 남긴다.
