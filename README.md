...
**안전장치**: 소스 하나가 응답 없어도 나머지로 계속 진행(`dead` 기록), 그날 수집이 0건이어도 에러 없이 "오늘은 조용합니다"로 끝까지 완주, 그래프 실행에 `recursion_limit`을 명시해 무한 실행을 방지. 자세한 내용은 [REPORT.md](REPORT.md) §4 참고.
...
├── tests/                      # pytest 유닛 테스트 (설정 로딩, 발행 텍스트 조립, 날짜 처리, 최소건수 보충, 지표 기록 인코딩)
