...
- 최소 발행 건수 보장: 과제 요구사항(3~5건)이 있는데, GPT에게 "최대 5건을 고르라"고만 하면 진짜 중요한 게 적을 때 1~2건만 돌아올 수 있다는 걸 실행하며 확인했다. `backfill_minimum()`을 추가해 본선 결과가 `MIN_TARGET`(3) 미만이면 예선 통과작 중 아직 안 뽑힌 것을 순서대로 채운다. 다만 예선 통과작 자체가 3건 미만이면 있는 만큼만 채우고 억지로 기사를 만들어내지는 않는다 — 실제로 이번 세션 테스트에서 예선 통과작이 2건뿐이었던 실행은 2건까지만 채워졌다.
...
- 실행마다 수집 건수·선별 건수가 달라질 수 있다는 것도 실제 반복 실행으로 확인했다: (a) 시간이 지나며 RSS 원본 콘텐츠 자체가 바뀌고, (b) `select()`가 LLM(GPT-4.1-mini, `temperature=0`)을 호출하는데 이것도 완전히 결정적이지는 않다. 같은 건수를 수집해도 선별 결과가 실행마다 다르게 나온 적이 여러 번 있었다.

### 측정 지표 — `store/metrics.jsonl` 누적 기록

`run()`이 실행마다 수집/선별/취재/검수통과 건수와 소스별 발행 기여를 `store/metrics.jsonl`에 한 줄씩 남긴다. 이번 세션에서 실제로 돌린 5회 기록을 집계하면:

| 지표 | 값 | 잰 방법 |
|---|---|---|
| 실행 횟수 | 5회 | `store/metrics.jsonl` 줄 수 |
| 평균 수집 건수 | 3.8건 | 각 실행의 `collected` 평균 |
| 평균 선별(발행) 건수 | 1.2건 | 각 실행의 `picked` 평균 |
| 선별율 (선별/수집) | 32% | `picked/collected`를 실행별로 구해 평균 |
| 검수 통과율 (발행/취재) | 100% | `published/drafted`를 실행별로 구해 평균 — 5회 모두 검수 탈락 없음 |

**주의**: 5회는 이번 세션에서 수동으로 돌려본 테스트 실행일 뿐, 실제 매일 자동 운영 데이터가 아니다. 수집 시간 창(24시간)에 새 기사가 얼마나 올라오느냐에 따라 결과가 크게 흔들리는 표본이라, 지금 수치를 "이 시스템의 정확도"로 해석하면 안 된다 — 매일 자동 실행이 쌓이면 이 표를 주기적으로 갱신해서 추세(예: 선별율이 계속 낮으면 소스나 기준을 재검토)를 보는 용도로 쓰는 게 맞다.

이 표를 만드는 과정에서 진짜 버그도 하나 발견했다: `run()`이 `store/metrics.jsonl`을 열 때 인코딩을 지정하지 않아서, 이 윈도우 컴퓨터에서는 cp949로 저장되고 있었다. 지금까지는 같은 컴퓨터에서만 읽고 써서 우연히 문제가 안 됐지만, UTF-8이 기본인 GitHub Actions(Linux)가 같은 파일에 이어 쓰기 시작하면 인코딩이 섞여서 파일이 깨졌을 것이다. `encoding="utf-8"`을 명시하도록 고치고, 기존 파일도 다시 인코딩해서 복구했다.
...
- 그래프 실행 시 `invoke(INIT, config={"recursion_limit": 15})`로 재귀(super-step) 한도를 명시적으로 걸었다. 지금 그래프는 순환 없이 선형(+팬아웃 한 번)이라 실제로 이 한도에 걸릴 일은 없지만, 나중에 재시도 루프 등을 추가했을 때 무한 실행을 막는 안전장치로 미리 걸어뒀다.
- "이상한 입력" 테스트: 그날 수집된 기사가 0건이면 어떻게 되는지 직접 확인했다. `select()`가 빈 목록을 받아도 예외 없이 `picked: []`를 돌려주고(예선 단계는 아예 건너뛰지만, 본선 단계는 빈 목록으로 실제 GPT 호출까지 정상적으로 이루어짐), 이후 `report`(팬아웃 0건)·`verify`(0→0건)·`publish`(`오늘은 조용합니다` 메시지)까지 전부 에러 없이 끝까지 완주한다. 수집 소스가 전부 죽거나 새 기사가 없는 날에도 파이프라인이 중간에 멈추지 않는다는 뜻이다.
- 네트워크 호출(RSS 수집, Google Chat 전송)에는 `timeout=20`초를 걸어뒀고, 소스 하나가 응답 없어도(`dead`) 나머지 소스로 계속 진행한다. GitHub Actions 잡 자체에도 `timeout-minutes: 20`이 걸려 있어 전체가 무한정 멈춰 있는 것도 방지된다.

## 5. 실행 기록

`python run.py`(DRY_RUN 없이, 실제 전송)를 여러 차례 실행해 실제 발행까지 확인했다. 최소 발행 건수 보장 로직 반영 후 마지막 실행 로그:

```
발행: 성공
① 수집   24시간 창 · 5건
② 선별   5 → 예선 2 → 1건 · 최소건수 보충 1건
   취재 완료 에듀동아 · 2027학년도 대기업 계약학과 수시모집
   취재 완료 베리타스알파 · 서울대·연세대·성균관대, 2026년
④ 검수   2 → 2건
⑤ 발행   2건 · 보냄
```
...
소스를 그냥 "찾았다"로 끝내지 않고, 후보로 나온 곳마다 실제로 접속해서 하나하나 확인한 것에 가장 공을 들였다. 교육부, KICE, 대교협, 정부 정책 사이트, 통계청, 커리어넷, 그리고 전국 시도교육청까지 후보를 다 뒤져봤는데, 특히 정부 공식 사이트들은 "RSS 제공"이라고 안내는 해놓고 실제로는 목록이 자바스크립트로 그때그때 채워지는 방식이라 우리 방식으로는 데이터를 가져올 수 없다는 걸 직접 확인해야 했다. 그 와중에 한국교육개발원과 서울시교육청은 실제로 쓸 수 있는 공식 RSS가 있다는 걸 찾아냈는데, 이게 이번 작업에서 가장 뿌듯한 부분이다 — 검색만으로는 안 나오고 사이트를 직접 열어서 메뉴 구조를 하나하나 뒤져봐야 나오는 정보였다.

두 번째로 공을 들인 부분은 선별 기준을 처음에 한 번 정하고 끝낸 게 아니라, 실제로 돌려본 결과를 보면서 계속 다시 고친 것이다. 베리타스알파의 수집 범위를 넓혔더니 "학자금대출 연체" 같은 입시와 상관없는 기사가 뽑히는 걸 발견해서 기준을 더 좁혔고, "대학 소식"이나 "고교 소식" 같은 분류는 만들어놨는데 정작 그걸 골라내는 기준이 없어서 애초에 통과가 안 되는 구멍이 있다는 것도 뒤늦게 알아채서 기준을 추가했다. "입시 일정이 바뀌었는가"처럼 매년 반복해서 나오는 정보를 "달라졌는가"로 물으면 정작 정기적으로 나오는 중요한 발표를 놓친다는 것도 직접 돌려보면서 깨닫고 "발표되었는가"로 다시 고쳤다. 처음 설계할 때는 전혀 안 보이던 문제들이었다.

**향후 보완하고 싶은 점**

가장 먼저 손대고 싶은 건 교육부·KICE 같은 공식 발표 소스다. KICE 사이트엔 RSS 안내가 있는데도 정작 열어보면 데이터가 안 나와서 아직 못 붙였다. 브라우저로 직접 열어서 어디서 데이터를 가져오는지 찾아내면 의외로 금방 추가할 수도 있을 것 같다.

두 번째는 서울 말고 나머지 시도교육청이다. 서울은 공식 RSS를 찾았지만 다른 지역은 사이트 구조가 제각각이라 아직 하나도 못 찾았다. 언론 검색으로 대신하는 방법도 있긴 한데, 공식 발표가 아니라 언론 보도라 성격이 달라서 더 고민해볼 부분이다.

세 번째는 `audience.yaml` 파일을 다시 점검하는 절차를 만드는 것이다. 지금은 이 설정 파일에 오타가 있어도 프로그램을 실제로 돌려보기 전까지는 알아채기 어렵고, 최악의 경우엔 엉뚱하게 동작하는 걸 보고 나서야 뒤늦게 깨닫게 된다. 실행하기 전에 필수 항목이 제대로 채워졌는지 한 번 미리 검사하는 장치를 넣어두면, 오타 때문에 헤매는 일을 줄일 수 있을 것 같다.

마지막으로 느낀 건, 같은 조건으로 여러 번 돌려도 매번 골라지는 기사 개수가 조금씩 달라진다는 점이다. AI가 판단하는 방식 자체가 매번 기계적으로 똑같은 답을 내놓지는 않다 보니 생기는 자연스러운 차이라고 지금은 받아들이고 있다. 다만 나중에 실제로 운영해보면서 이 차이가 너무 크게 느껴진다면, 기사를 추리는 방식(한 번에 몇 건씩 묶어서 보여줄지, 어떻게 질문할지)을 다시 손볼 필요가 있을 것 같다.
