...
Google Chat 스페이스에 실제로 "🗞️ 2026-09-15 · 대입·교육정책 브리핑" 메시지가 도착했고, 토픽 이모지(📊)와 "원문 보기 · 입시통계·데이터 · 에듀동아" 형식의 하단 표기까지 정상 렌더링됨을 육안으로 확인했다.

<!-- 위 로그만으로는 실제 Chat 화면의 렌더링(굵게·링크·이모지가 실제로 어떻게 보이는지)까지는 증명이 안 되니, Google Chat 스페이스 화면 캡처와 `python metrics_report.py` 출력 캡처를 이 아래에 이미지로 첨부할 것 -->

(Google Chat 실제 화면 캡처 삽입 예정)
